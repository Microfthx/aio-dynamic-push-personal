# aio-dynamic-push-personal

基于上游项目 [`nfe-w/aio-dynamic-push`](https://github.com/nfe-w/aio-dynamic-push) 的个人修改版，保留原许可证，并在此基础上按个人使用场景做定制。
...
- `start.sh`：启动项目，优先使用 `.venv/bin/python`
- `restart.sh`：一键重启，优先重启 `systemd` 服务，否则重启当前后台进程
- `install_systemd_service.sh`：安装 `systemd` 服务并设置开机自启

使用方式：

```sh
cd /path/to/aio-dynamic-push-personal
chmod +x start.sh install_systemd_service.sh
sudo bash install_systemd_service.sh
```

如果暂时不安装开机自启，也可以直接用下面的脚本一键重启：
...
```sh
systemctl status aio-dynamic-push
journalctl -u aio-dynamic-push -f
systemctl restart aio-dynamic-push
systemctl stop aio-dynamic-push
```
